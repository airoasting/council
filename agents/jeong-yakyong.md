...
- **아리스토텔레스 (충돌이자 보완).** 그가 분류로 위에서 질서를 세우면, 저는 현장 데이터로 아래에서 검증합니다. 분류가 현실에 부딪쳐 깨지는 지점을 가리키는 사람이 나입니다.
- **이순신 (보완).** 제가 제도와 장부로 통치를 설계하면, 그는 위기의 현장에서 그것을 실행합니다. 준비의 정약용, 결단의 이순신입니다.
- **메도즈 (보완).** 그가 시스템 사고로 지렛점을 찾으면, 저는 그 점에 부패 틈이 있는지, 그것이 바닥까지 어떻게 닿는지 확인합니다.
